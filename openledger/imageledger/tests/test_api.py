...
class TestAPIViews(TestImageledgerApp):
    ...
    def test_list_delete_nonexistent_image(self):
        ...
        resp = self.delete('/api/v1/lists', {'slug': 'unknown'})
        assert 404 == resp.status_code
    ...
    def test_list_delete(self):
        ...
        resp = self.delete('/api/v1/lists', {'slug': lst.slug})
    ...
    def test_lists_create_no_title(self):
        ...
        resp = self.client.post('/api/v1/lists', {'title': ""})
        self.assertEquals(422, resp.status_code)
    ...
    def test_lists_create_list(self):
        ...
        title = 'my list title'
        assert 0 == models.List.objects.filter(title=title).count()
        resp = self.client.post('/api/v1/lists', {'title': title})
    ...
    def test_lists_modify_list(self):
        ...
        resp = self.put('/api/v1/lists', {'slug': lst.slug, 'identifier': [img1.identifier, img2.identifier]})

        assert 200 == resp.status_code
        self.assertEquals(2, models.List.objects.get(title=title).images.count())

        # Now "delete" one image
        resp = self.put('/api/v1/lists', {'slug': lst.slug, 'identifier': [img1.identifier]})
        assert 200 == resp.status_code
    ...
    def test_lists_create_while_modifying(self):
        ...
        title = 'my list title'
        resp = self.put('/api/v1/lists', {'title': title, 'identifier': []})
        assert 201 == resp.status_code
        assert 1 == models.List.objects.filter(title=title).count()
    ...
    def test_lists_create_list_with_images(self):
        ...
        resp = self.client.post('/api/v1/lists', data={'title': title, 'identifier': [img1.identifier, img2.identifier]})
        assert 201 == resp.status_code
    ...
    def test_get_lists_by_title(self):
        ...
        img1 = models.Image.objects.create(title='image title', url='http://example.com/1', license='CC0')
        img2 = models.Image.objects.create(identifier=img2.identifier, title='image title', url='http://example.com/2', license='CC0')
        ...
        resp = self.client.get('/api/v1/lists?title=' + title1)
        assert 200 == resp.status_code
        ...
        resp = self.client.get('/api/v1/lists?title='  + title2)
        assert 200 == resp.status_code
        assert 'lists' in resp.json()
    ...
    def test_get_lists_by_title(self):
        ...
        resp = self.client.get('/api/v1/lists?title=not+found')
    ...
    def test_get_all_lists_by_title(self):
        ...
        resp = self.client.get('/api/v1/lists?title='  + match)
        assert 200 == resp.status_code
        assert 'lists' in resp.json()
